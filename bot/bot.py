...
def has_badge_art(r):
    """Есть ли НАСТОЯЩИЙ арт значка (а не карточка с одним названием).

    Владелец просил: без картинки в канал не постить. Карточка-заглушка с
    текстом (её рисуем, когда кампания анонсирована, а значка ещё нет) выглядит
    сломанной — «Clipped That» ушёл именно так. Значка нет ни у SD, ни у Twitch:
    кампания идёт, а объект ещё не создан, взять арт неоткуда. Ждём."""
    key = collector.image_cache_key(r.get("image") or "")
...
async def publish_new(context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, r in live.items():
        w = r.get("window") or {}
        # Без настоящего арта значка в канал не идём. Записи в state тоже не
        # заводим — значит, когда Twitch наконец выложит картинку, значок
        # объявится обычным порядком, а не потеряется навсегда.
        if not has_badge_art(r):
            continue
        end = effective_end(w)
    ...
    # Поста «раздача завершилась» намеренно НЕТ: владелец попросил убавить
    # уведомления, а этот был наименее полезным — он сообщает о том, чего уже
    # нельзя сделать. Последним словом остаётся «последний день», и оно как раз
    # зовёт к действию.

    night = in_quiet_hours(now)

    # ПОЯВЛЕНИЯ/СТАРТЫ: по ОДНОЙ ГРУППЕ за тик — так они расходятся во времени,
    # а не валятся пачкой. Ночью несрочные анонсы (старт >суток) придерживаем до
    # утра — если тихие часы включены (сейчас нет: QUIET_HOURS_START==END в .env,
    ...
    for (gk, kind), items in sorted(buckets.items()):
        ...
        try:
            if len(items) == 1:
                ...
                # Картинки может не быть в принципе — например, «раздача
                # завершилась» шлётся, когда значок уже не актуален, а его
                # карточку sync_cards к тому времени удаляет. Такой пост
